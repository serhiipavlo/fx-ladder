...
> **Ціль розгортання:** Azure (Static Web Apps + Container Apps).
...
- **Живий демо-лінк на Azure**: рекрутер відкриває URL і бачить справжні WebSockets через справжню мережу — з реальними проксі, таймаутами й реконектами, а не їх імітацією на localhost.
...
| **CI/CD** | GitHub Actions: тести → coverage-гейт → перф-гейт → деплой у SWA + Container Apps; PR-preview середовища SWA (розд. 9, 10) | Розмова «як тримати NFR зеленими» + справжній пайплайн до хмари, а не «працює локально» |
...
`sim-core` має рівно один продакшн-хост — Node усередині feed-server. Первісний задум тримав другий хост — Web Worker у браузері (офлайн-демо без сервера, локальний хост для E2E); після переїзду демо на Azure його **знято повністю** (ревізія ADR-03). Кожну з ролей другого хоста закриває дешевший механізм:
...
2. **Фолбек співбесіди** — той самий сервер, запущений локально (`docker run` або dev-режим): якщо мережа закладу чи Azure відмовили, демо йде з localhost;
...
|---|---|---|
| `apps/web` | статичний хостинг із CDN і TLS (Azure Static Web Apps, Free) | статиці не потрібен сервер; **PR-preview середовища** з коробки — рецензування з живого лінка, а не зі скріншотів |
| `apps/feed-server` | один контейнер у керованому середовищі (Azure Container Apps) | контейнер = відтворюване середовище; WebSockets працюють через ingress нативно |
...
Конкретна платформа тут свідомо **не є архітектурним рішенням**: потрібні статика з CDN, один контейнер із публічним HTTPS/WS і пайплайн, який їх оновлює; усе це є в будь-якого хмарного провайдера. Azure узятий тому, що акаунт уже є (ADR-11).
...
flowchart TB
    subgraph GH["GitHub"]
        ...
        CI --> WD["web: build → deploy у SWA<br/>PR → тимчасовий preview-URL"]
        CI --> IMG["server: docker build<br/>linux/amd64, multi-stage → push у ghcr"]
        IMG --> UPD["az containerapp update"]
    end
    subgraph AZ["Хмара"]
        SWA["статичний хостинг<br/>статика · CDN · TLS"]
        ACA["контейнер із feed-server<br/>публічний HTTPS + WS"]
    end
    WD --> SWA
    UPD --> ACA
    U["Браузер користувача"] -- "https: статика застосунку" --> SWA
    U -- "wss /feed · wss /graphql<br/>https /api · /sim — напряму" --> ACA
```
...
**Чому E2E не проти Azure.** CI-тест проти хмари змішує дві незалежні речі: регресію коду і погоду мережі/платформи (латентність, холодні старти, ліміти). Червоний тест має означати «зламали код» — завжди і тільки. Тому E2E у CI бігає проти локального feed-server (хмарної мережі в контурі немає — немає і її флейків), а живий Azure перевіряється окремим smoke-чеком після деплою, чия червоність означає «зламали деплой».
...
**ADR-05 · Warm plane — GraphQL на тому ж порту через graphql-ws** *(розвилка 2 — «так», ревізовано під Azure)*.
...
Контекст: жоден FR не вимагає переживати рестарт сервера. Рішення: увесь стан — у процесі; рестарт = новий торговий день; блоттер живе в процесі й у кеші Apollo. БД додала б міграції, серіалізацію, ще один сервіс у деплої — вартість без покупця. Рестарт контейнера — деплой, перезапуск платформи — це кінець торгового дня, і це **задокументована поведінка демо**, а не баг. Ціна: історія втрачається на рестарті — за скоупом це не втрата.

**ADR-11 · Розгортання: статика на CDN + один контейнер.**
Контекст: демо мусить відкриватись лінком і показувати справжню мережу (розд. 9). Рішення: статичний хостинг із CDN і PR-preview для `apps/web`, один контейнер із feed-server, образи в ghcr, деплой із GitHub Actions. Конкретний провайдер — Azure (Static Web Apps + Container Apps) — узятий тому, що акаунт уже є; архітектурно на його місці міг би стояти будь-який хостинг статики плюс будь-яке середовище запуску контейнерів, і жодне рішення документа від заміни не змінюється. Відкинуто: статика без бекенда (GitHub Pages і симуляція в браузері) — транспорт несправжній, жодного реального WS через проксі; кластер чи власна VM — адміністрування без виграшу для жодного FR. Ціна: пайплайн прив'язаний до CLI конкретної платформи — переїзд коштує переписування одного workflow-файла.
...
|---|---|---|
| Публічний control plane | Хтось чужий крутить демо | Радіус ураження мінімізовано конструкцією: команди впливають лише на симуляцію, дані синтетичні; ліміт одночасних клієнтів; Origin-allowlist (7.1); за потреби — секрет у заголовку для `/sim/*` — година роботи |
